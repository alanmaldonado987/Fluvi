...
function FormCobrar({ deuda, onGuardar }) {
  const { state } = useFinance()
  const [billetePagoId, setBilletePagoId] = useState(null)
  const [fecha, setFecha] = useState(hoy())
  const billeteras = state.billeteras.map((b) => ({ value: b.id, label: b.nombre }))
  const valido = billetePagoId && fecha

  const enviar = (e) => {
    e.preventDefault()
    onGuardar(deuda.id, billetePagoId, fecha)
  }

  return (
    <form className="grid gap-4" onSubmit={enviar}>
      <p className="text-sm text-muted-foreground">
        <strong>{deuda.persona}</strong> te paga <strong>${deuda.valor.toLocaleString('es-CO')}</strong>.
      </p>
      <div className="grid grid-cols-2 gap-3">
      ...
      <DialogFooter>
        <DialogClose render={<Button variant="outline" />}>Cancelar</DialogClose>
        <Button type="submit" disabled={!valido}>Registrar pago</Button>
      </DialogFooter>
    </form>
  )
}

export function DeudaDialog({ open, onOpenChange, modo = 'prestar', deuda, onPrestar, onCobrar }) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-sm:top-auto max-sm:bottom-0 max-sm:max-w-full max-sm:translate-y-0 max-sm:rounded-b-none max-sm:duration-300 max-sm:ease-drawer max-sm:data-open:zoom-in-100 max-sm:data-open:slide-in-from-bottom-8 max-sm:data-closed:zoom-out-100 max-sm:data-closed:slide-out-to-bottom-8">
        ...
        {modo === 'cobrar' && deuda ? (
          <FormCobrar deuda={deuda} onGuardar={(...args) => { onCobrar(...args); onOpenChange(false) }} />
        ) : (
